...
const {
  dialog, Menu, MenuItem, getCurrentWebContents,
...
const menuItem = new MenuItem({
  ...
  click: () => {
    ...
    document.querySelector(`#collections-content [data-id='${global._rightClickedStickerId}']`).parentElement.remove();
    document.getElementById('sub-collections-content').remove();
  },
});
...
function displayCovers() {
  const ele = document.getElementById('collections-content');
  ele.innerHTML = '';
  window.funcs.recFindByExt(getCoverDir(), 'png').forEach((src) => {
...
function displayStickers(src) {
  const id = /(\d+)\.png/g.exec(src)[1];
  const ele = document.getElementById('sub-collections-content');
  ele.innerHTML = '';
  window.funcs.recFindByExt(`${store.get('downloadDir', defaultDownloadDir)}/${id}`, 'png').forEach((path) => {
    ele.innerHTML += `<div class="col-sm-2"><img src="${path}" width="70%"/></div>`;
  });
  setTimeout(() => {
    document.querySelectorAll('#sub-collections-content img').forEach((event) => {
      event.ondragstart = (event) => {
        event.preventDefault();
        const src = event.target.getAttribute('src');
        const target = window.funcs.apng2gif(src);
...
function renderPopularStickers() {
  const ele = document.getElementById('popular-stickers');
  ele.innerHTML = spinner;
  const p = window.funcs.getRandomPopular();
  p.then((data) => {
    ele.innerHTML = '';
...
function searchStickers() {
  ...
  const ele = document.getElementById('stickers');
  ele.innerHTML = spinner;
  p.then((data) => {
    ele.innerHTML = '';
...
function showStickers(id) {
  ...
  const ele = document.getElementById('sub-stickers');
  ele.innerHTML = spinner;
  p.then((data) => {
    global.urls = data;
    ele.innerHTML = '';
...
function showDownloadProgress() {
  ...
  const monitor = setInterval(() => {
    ...
    const percentage = `${Math.round((global._downloaded / global.urls.length) * 100)}%`;
